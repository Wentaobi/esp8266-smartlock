...
def get_AP_info(): #currently disabled
    ssid=''
    password=''
...
def set_esp(netconfig):
    print('default server: ')
    print(host_url)
    res = requests.get("http://192.168.4.1/"+netconfig['ssid']+"="+netconfig['password']+"="+netconfig['url']) #192.168.4.1 is esp8266's default ip address when it serves as an access point. the requests sends ssid, PSK and the location of HTTP server to esp8266
    #还需加入超时检测
    return(res.text)

print("connect to AP:ESP8266 and press inter") #instruct the user to connect to the Soft Access Point esp8266 created
no_sense=input()
set_esp(wificonfig.netconfig)
